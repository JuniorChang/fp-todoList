    ...
    items.forEach((item) => {
        ...
        if (item.querySelector(".title").getAttribute('data-time') == itemData.addedAt) {
            ...
            item.querySelector("[data-delete]").addEventListener("click", function (e) {
                e.preventDefault();
                if (confirm("Jr ask do you really want to remove this task?")) {
                    itemsList.removeChild(item);
                    removeItem(item);
                    setLocalStorage(todoItems);
                    alertMessage("Jr says task has been removed.", "alert-success");
...
document.addEventListener("DOMContentLoaded", () => {
    form.addEventListener("submit", (e) => {
        ...
        if (itemName.length === 0) {
            alertMessage("Jr says enter a task first before adding", "alert-danger");
        }
        else {
            ...
            if (currentItemIndex) {
                ...
                alertMessage("Jr has updated the task for you", "alert-success");
            }
            else {
                const itemObj = {
                    name: itemName,
                ...
                setLocalStorage(todoItems);
                alertMessage("Jr has added a new task for you", "alert-success");
            }
            getList(todoItems);
        }
        itemInput.value = "";
